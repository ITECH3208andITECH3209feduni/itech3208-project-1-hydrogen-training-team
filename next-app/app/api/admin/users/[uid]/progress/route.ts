...
export async function GET(
    ...
    try {
        ...
        const completedModules = moduleProgress?.filter((item) => item.status === "done" || Number(item.progress) >= 100).length ?? 0;
        ...
        const quizScores = quizProgress
            ?.map((item) => Number(item.score)).filter((score) => !Number.isNaN(score)) ?? [];
...
export async function PATCH(
    ...
    try {
        ...
        const { module_id, section, status } = body;

        if (!module_id || !section || !status) {
            return NextResponse.json(
                { ok: false, error: "module_id, section and status are required" },
                { status: 400 }
            );
        }
