  ...
  async function loadUsers() {
    try {
      ...
      if (data.ok) {
        ...
        const learners = loadedUsers.filter((user) =>
          user.role !== "admin" && user.user_type === "public"
        );

        const progressResults = await Promise.all(learners.map(async (learner) => {
          try {
            const progressResponse = await fetch(`/api/admin/users/${learner.uid}/progress`, {
              method: "GET",
              cache: "no-store",
              headers: {
                Authorization: `Bearer ${idToken}`,
            ...
            if (!progressData.ok || !progressData.summary) {
              return null;
            }

            return progressData.summary;
          } catch (error) {
            console.error(`Failed to load progress for ${learner.uid}:`, error);
            return null;
          }
        }));
        ...
        const completedCount = validProgressResults.filter((result) =>
          result.completedModules >= result.totalModules && result.quizPassed
        ).length;

        const average = validProgressResults.length > 0
            ? Math.round(validProgressResults.reduce((total, result) =>
              total + Number(result.overallProgress), 0
      ...
      <div className="admin-stat-cards">
        ...
        <div className="stat-card completed-card">
          <div className="stat-info">
            ...
            <div className="count">{completedTraining}</div>
            <div className="sub">Learners Completed</div>
          </div>
        </div>
        ...
        <div className="stat-card progress-card">
          <div className="stat-info">
            ...
            <div className="count">{averageProgress}%</div>
            <div className="sub">Across All Learners</div>
          </div>
        </div>
      </div>
          ...
          <table className="admin-table">
            ...
            <tbody>
              {filteredUsers.length === 0 ? (
                <tr>
                  <td colSpan={6} className="no-results">No users match your search.</td>
                </tr>
              ) : (
