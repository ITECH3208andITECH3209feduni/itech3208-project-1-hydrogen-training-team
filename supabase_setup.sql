...
-- lab-images: the single background image behind the lab's hotspots.
...
-- lab-videos: one embedded video file per lab hotspot, same mp4-only scope as module-videos.
insert into storage.buckets (id, name, public)
...
--
-- Note: `enable row level security` + a policy only controls which *rows* a role can see/touch once it already has the base table-level privilege for that operation — it does not grant that privilege itself.
-- Tables created via the SQL Editor (rather than the dashboard's table UI) don't get this automatically, so without the explicit `grant`, every matching query fails with `permission denied for table ...` (Postgres 42501) even though the policy looks correct. 
-- Hence the grant lines below, per table and per role.
--
-- The service_role Postgres role has BYPASSRLS set, so it never actually needs an RLS policy to read or write anything.
...
-- Storage bucket access rules live on storage.objects rather than needing RLS enabled or a grant select of their own.
--
-- None of the three buckets below get a public-read policy:
-- A public bucket serves individual objects through its own public URL endpoint (getPublicUrl), which never checks RLS at all.
-- A SELECT policy on storage.objects only matters for listing objects via the storage API, which this app never does — adding one here would only let anon enumerate every file in the bucket, with no upside.
--
-- Policy names must be unique per table, not per bucket, so each one below is disambiguated with its bucket name even though storage.objects is shared by all three.

-- lab-images: only ever uploaded/overwritten, never deleted, so no delete policy.
create policy "Allow service role upload (lab-images)"
on storage.objects
for insert
to service_role
with check (bucket_id = 'lab-images');

create policy "Allow service role update (lab-images)"
on storage.objects
for update
to service_role
using (bucket_id = 'lab-images');

-- module-videos: uploaded, replaced, and removed by app/api/modules/video/route.ts.
create policy "Allow service role upload (module-videos)"
on storage.objects
for insert
to service_role
with check (bucket_id = 'module-videos');

create policy "Allow service role update (module-videos)"
on storage.objects
for update
to service_role
using (bucket_id = 'module-videos');

create policy "Allow service role delete (module-videos)"
on storage.objects
for delete
to service_role
using (bucket_id = 'module-videos');

-- lab-videos: uploaded, replaced, and removed by app/api/lab/video/route.ts.
create policy "Allow service role upload (lab-videos)"
on storage.objects
for insert
to service_role
with check (bucket_id = 'lab-videos');

create policy "Allow service role update (lab-videos)"
on storage.objects
for update
to service_role
using (bucket_id = 'lab-videos');

create policy "Allow service role delete (lab-videos)"
on storage.objects
for delete
to service_role
using (bucket_id = 'lab-videos');
